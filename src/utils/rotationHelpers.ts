...
function rotatePoint(lon: number, lat: number, centerLon: number, centerLat: number): [number, number] {
...
function mapCoords(coords: any, fn: (p: [number, number]) => [number, number]): any {
...
export function rotateFeatureCollection(
  ...
  const fn = (p: [number, number]) => rotatePoint(p[0], p[1], centerLon, centerLat);
  ...
  return {
    ...
    features: fc.features.map((f) => {
      ...
      return {
        ...
        geometry: {
          ...g,
          coordinates: mapCoords(g.coordinates, fn),
        },
      };
    }),
  };
